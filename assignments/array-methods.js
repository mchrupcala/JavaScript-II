...
let fullName = [];
runners.forEach(function(bibs){
   fullName.push(bibs.first_name + " " + bibs.last_name);
})
...
let largeShirts = [];
largeShirts = runners.filter(function(big){
...
ticketPriceTotal = runners.reduce(function(acc, price){
...
// Problem 1: How many of these runners are students?
//use filter for '.edu'
let students = [];

students = runners.filter(function(school){
    return school.email.includes(".edu");
})

console.log(students);

// Problem 2: Who donated $100 or more?
let donors = [];

donors = runners.filter(function(spender){
    return spender.donation > 99;
})
console.log(donors);
